...
//Dati di rete WiFi
const char* ssid = "IoT@msr";                     //"Syde Technology";  //Massimo: "VodafoneMinelli"
const char* password = "49198232";                    //"nVvffsb#95B!";  //Massimo: "1189sdafoS!"

/*
//Indirizzi statici assegnati all'ESP32
IPAddress local_IP(192,168,0,101);                     //(192, 168, 10, 111);  //Massimo: (192, 168, 1, 111)
IPAddress gateway(192,168,0,1);                      //(192, 168, 10, 1);     //Massimo: (192, 168, 1, 1)
IPAddress subnet(255, 255, 255, 0);
IPAddress primaryDNS (192,168,0,1);                   //(192, 168, 10, 200);  //Massimo (192, 168, 1, 1)
IPAddress secondaryDNS(8, 8, 8, 8);    //(oppure (8, 8, 4, 4))
*/
...
void outputToDAC(Adafruit_MCP4725& dac, float Vd) {
  ...
  if (Vd_constrained >= -B && Vd_constrained <= B){
    ...
    scaled = (Vd_mV / 3000.0) * 4095.0;
  }
  //Serial.printf("scaled: %.2f\n", scaled);
  dac.setVoltage((uint16_t)scaled, false);
}

//Converte una tensione compresa tra -1500mV a +1500mV (Vd) in una tensione 0-3300mV per il DAC
void outputToDACTEST(Adafruit_MCP4725& dac, float VdTEST) {
  ...
  if (VdTEST_constrained >= -B && VdTEST_constrained <= B){
    VdTEST = 1500;
  }
  if (VdTEST > 3300.0){
    VdTEST = 3300.0;
...
/**
 * @brief Initializes two HX711 load cells and loads stored preferences.
 * 
 * This function sets up communication with two HX711 ADCs connected to load cells.
 * It also calls the function to load saved calibration or system preferences.
 * 
 * @return int Always returns 0 after setup is complete.
 */
int HX711Setup(){
  // HX711 Setup
  scale1.begin(dataPin1, clockPin1);   // initialize first load cell
  scale2.begin(dataPin2, clockPin2);   // initialize second load cell
  loadPrefs();                         // load calibration and system preferences
  return 0;
}

/**
 * @brief Initializes two MCP4725 DACs on separate I2C buses and configures input pins.
 * 
 * This function initializes two I2C buses with specified SDA/SCL pins and 
 * begins communication with two DACs using the MCP4725 library. It also sets
 * the DAC feedback pins as inputs for voltage reading or monitoring.
 * 
 * @return int Always returns 0 after setup is complete.
 */
int DACSetup(){
  // DAC Initialization
  I2C_1.begin(SDA1_Pin, SCL1_Pin);          // start first I2C bus
  I2C_2.begin(SDA2_Pin, SCL2_Pin);          // start second I2C bus
  dac1.begin(0x60, &I2C_1);                 // initialize DAC1 at address 0x60 on I2C_1
  dac2.begin(0x60, &I2C_2);                 // initialize DAC2 at address 0x60 on I2C_2 (may need different address if not mirrored)
  
  pinMode(dacInputPin1, INPUT);            // set DAC1 feedback pin as input
  pinMode(dacInputPin2, INPUT);            // set DAC2 feedback pin as input
  return 0;
}

/**
 * @brief Sets the color of a WS2812B addressable LED using the FastLED library.
 * 
 * This function initializes the LED strip and sets the first LED to the specified color.
 * 
 * @param RGBValue A CRGB color value (e.g., CRGB::Red, CRGB(255, 165, 0), etc.)
 * 
 * @return int Always returns 0 after setting the LED color.
 */
int LEDSetColour(CRGB RGBValue){
  // Developer feedback LED
  FastLED.addLeds<WS2812B, LED_PIN, GRB>(leds, NUM_LEDS);  // initialize LED strip
  leds[0] = RGBValue;                                      // set first LED to specified color
...
void setup() {
  //Serieller Monitor Konfigurieren
  Serial.begin(115200);
  delay(5000);     //Delay for Serial Monitor start


  HX711Setup();
  DACSetup();
  LEDSetColour(CRGB::Yellow);
  ...
  //Call back funktionen werden gesetzt
...
void loop() {
  ...
  //Serial.printf("p: %d, Kp1: %.2f, Kd1: %.2f, Kp2: %.2f, Kd2: %.2f, B: %.2f\n", p, Kp1, Kd1, Kp2, Kd2, B);     //DEBUG
