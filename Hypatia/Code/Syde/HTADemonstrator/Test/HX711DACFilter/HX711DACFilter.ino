...
float alpha1 = 0.1;
float alpha2 = 0.1;
float filteredWeight1 = 0.0;
float filteredWeight2 = 0.0;
...
void handleSetAlpha1() {
    if (server.hasArg("value")) {
        alpha1 = server.arg("value").toFloat();
        server.send(200, "text/plain", String((int)(alpha1 * 100)) + "% smoothing (alpha1)");
    } else {
        server.send(400, "text/plain", "Missing value");
    }
}

void handleSetAlpha2() {
    if (server.hasArg("value")) {
        alpha2 = server.arg("value").toFloat();
        server.send(200, "text/plain", String((int)(alpha2 * 100)) + "% smoothing (alpha2)");
    } else {
        server.send(400, "text/plain", "Missing value");
    }
}

String getAlpha1Percent() { return String((int)(alpha1 * 100)); }
String getAlpha2Percent() { return String((int)(alpha2 * 100)); }
...
float start1() {
    ...
    filteredWeight1 = alpha1 * raw + (1 - alpha1) * filteredWeight1;
    return filteredWeight1;
}

float start2() {
    float raw = myScale2.get_units(10);
    filteredWeight2 = alpha2 * raw + (1 - alpha2) * filteredWeight2;
    return filteredWeight2;
}
...
void handleRoot() {
    ...
    <body>
      ...
      <script>
            ...
            function updateAlpha1(val) {
                let alpha = 1 - (val / 100);
                document.getElementById("alpha1val").innerText = alpha.toFixed(2);
                document.getElementById("alpha1percent").innerText = "Current: " + val + "%";
                fetch('/setalpha1?value=' + alpha);
            }

            function updateAlpha2(val) {
                let alpha = 1 - (val / 100);
                document.getElementById("alpha2val").innerText = alpha.toFixed(2);
                document.getElementById("alpha2percent").innerText = "Current: " + val + "%";
                fetch('/setalpha2?value=' + alpha);
            }
...
void setup() {
   ...
   // MCP4725_2.begin(0x61, &Wire); // Default I2C Address of MCP4725 breakout board (sparkfun)  
    ...
    server.on("/setalpha1", handleSetAlpha1);//Filter
    server.on("/setalpha2", handleSetAlpha2);
