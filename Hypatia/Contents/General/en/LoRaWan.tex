...
\subsection{PHY Payload (\PYTHON{PHYPayload})}
	Der \PYTHON{PHYPayload} besteht aus Mac Header (\PYTHON{MHDR}), \hyperref[lorawan:macPayload]{Mac Payload (\PYTHON{MACPayload})} und \hyperref[lorawan:mic]{\acl{mic} (\PYTHON{\acs{mic}})} 
...
\subsubsection{Message Integrity Code (\PYTHON{\acs{mic}}) \label{lorawan:mic}}  
...
\subsubsection{Frame-Payload (\PYTHON{FRMPayload}) \label{lorawan:frmpayload}}
	...
	Wenn \PYTHON{FPort} gleich 0 ist, werden in \PYTHON{FRMPayload} ausschlie{\ss}lich \hyperref[lorawan:macCommands]{\ac{mac}-Befehle} übertragen, die zur Verwaltung der Netzkommunikation dienen. Sie werden mithilfe des \hyperref[lorawan:NwkSKey]{\PYTHON{\acs{nwkskey}}} verschlüsselt.  
	...
	Es ist zu beachten, dass dieses Feld verschlüsselt werden muss, bevor der \hyperref[lorawan:mic]{\PYTHON{\ac{mic}}} berechnet wird \cite{LoRaAlliance:2020b, Montagny:2022}.


% tikz verschlüsseln
\subsection{Frame-Header (\PYTHON{FHDR}) \label{lorawan:frameHeader}}
	Die \PYTHON{FHDR} besteht aus der Endgeräteadresse \hyperref[lorawan:deviceAddr]{\PYTHON{\acs{devaddr}}}, dem Frame-Control-Byte \PYTHON{FCtrl}, einem 2 Bytes langen Frame Counter (\PYTHON{FCnt}) sowie bis zu 15 Bytes an Frame Options (\PYTHON{FOpts}).
...
\subsubsection{\PYTHON{FCtrl} für downlink Frames}
	...
	\begin{itemize}
		\item \PYTHON{ADR} wird verwendet, um die adaptive Datenrate \PYTHON{\acs{adr}} zu aktivieren oder zu deaktivieren (siehe~\ref{lorawan:adr}).
...
\subsubsection{\PYTHON{FCtrl} für uplink Frames}
	...
	\begin{itemize}
		\item \PYTHON{ADR} und \PYTHON{ADRACKReq} werden bei der Verwendung der adaptiven Datenrate \PYTHON{\acs{adr}} eingesetzt (siehe~\ref{lorawan:adr}).
...
\section{Verschlüsselung und Sicherheit}
	...
	\subsubsection{\acl{deveui} \PYTHON{\acs{deveui}}}
	...
	\subsubsection{\acl{joineui} \PYTHON{\acs{joineui}}}
	...
	\subsubsection{\acl{devaddr} \PYTHON{\acs{devaddr}} \label{lorawan:deviceAddr}}
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[scale=0.59, every node/.style={transform shape}]
			...
			\draw[arrowStyle, <->, green!70!black] (3, 0) -- node[midway, above] {\PYTHON{\acs{nwkskey}}} (14, 0);
			\draw[arrowStyle, <->, orange!70!black] (3, -1) -- node[midway, above] {\PYTHON{\acs{nwkskey}}} (19.5, -1);
		\end{tikzpicture}
...
\subsection{\acf{abp}}
	Die Sitzungsschlüssel (\PYTHON{\ac{appskey}} und \PYTHON{\ac{nwkskey}}) sowie die \PYTHON{\ac{devaddr}} sind dauerhaft im Gerät und auf dem Netzwerkserver gespeichert und werden nicht dynamisch erzeugt. Diese Schlüsseln werden bei jeder Sitzung verwendet. Allerdings besitzt jedes Endgerät individuelle Sitzungsschlüssel, sodass im Falle einer Kompromittierung nicht automatisch alle Geräte betroffen sind \cite[48]{Semtech:2024}. Diese Methode ist zwar einfach in der Handhabung, bietet jedoch ein geringeres Sicherheitsniveau, da die Schlüssel nicht regelmä{\ss}ig erneuert werden \cite{Montagny:2022}.

\subsection{\acf{otaa}}
	Bei der \ac{otaa} werden die Sitzungsschlüssel dynamisch generiert, wodurch jede Verbindung eine eigene, eindeutige Sicherheitssitzung erhält. Dies erhöht die Datensicherheit erheblich, da bei jeder erneuten Aktivierung neue Schlüsselpaare (\PYTHON{\ac{nwkskey}} und \PYTHON{\ac{appskey}}) abgeleitet werden. Die Schlüssel werden auf Basis des Hauptschlüssels (\PYTHON{\ac{appkey}}) gebildet, der sowohl auf dem Endgerät als auch auf dem Join Server hinterlegt ist. Auf diese Weise wird gewährleistet, dass die übertragene Kommunikation zwischen Endgerät und Netzwerk stets kryptografisch geschützt und voneinander isoliert bleibt \cite{LoRaAlliance:2020b, Semtech:2024, Montagny:2022}

\subsection{Weitere Parameter bei Verwendung von \ac{otaa}}
	\begin{itemize}
		\item \textbf{\acl{devnonce} \PYTHON{\ac{devnonce}} - Die \PYTHON{\ac{devnonce}} ist ein Zähler, der beim Start des Systems mit dem Wert~0 beginnt und sich bei jeder \textit{Join Request}-Nachricht um~1 erhöht. Wird das Endgerät regelmä{\ss}ig ausgeschaltet, muss die \PYTHON{\ac{devnonce}} in einem nichtflüchtigen Speicher (\textit{non-volatile memory}) gesichert werden, damit der Zählerwert nach einem Neustart erhalten bleibt. Wird die \PYTHON{\ac{devnonce}} zurückgesetzt oder die \PYTHON{\ac{joineui}} geändert, verwirft der JoinServer alle \textit{Join Request}-Nachrichten, da er die zuletzt verwendete \PYTHON{\ac{DEVNONCE}} jedes Geräts speichert, um Wiederholungsangriffe zu verhindern \cite[45]{LoRaAlliance:2020b}.
		\item \textbf{\acl{joinnnonce}} \PYTHON{\ac{joinnonce}}- Eine Zufallszahl, die während des Join-Prozesses vom Join Server erzeugt wird. Sie wiederholt sich nie und wird verwendet, um nach der Join-Prozedur die \PYTHON{\ac{nwkskey}} und \PYTHON{\ac{appskey}} zu generieren \cite[46]{LoRaAlliance:2020b}.
	\end{itemize}

\subsection{Join-Prozedur}
	Zu Beginn müssen dem Gerät der \PYTHON{\ac{appkey}}, die \PYTHON{\ac{joineui}} und die \PYTHON{\ac{deveui}} bekannt sein. Die Join-Prozedur besteht aus zwei MAC-Paketen: \textit{Join Request} und \textit{Join Accept}.
...
\subsubsection{Berechnung der Sitzungsschlüssel}
	Im nächsten Schritt berechnet das Endgerät auf Basis von \PYTHON{\ac{deveui}}, \PYTHON{\ac{joineui}}, \PYTHON{\ac{devnonce}}, den Hauptschlüssel (\PYTHON{\ac{appkey}}) sowie den Feldern der \textit{Join Request}- und \textit{Join Accept}-Nachrichten seine Sitzungsschlüssel. Der JoinServer berechnet dieselben Schlüssel und teilt sie mit dem \ac{lns} und dem Anwendung Server, um die Kommunikation zwischen Endgerät, Netzwerk- und Anwendungsserver abzusichern.
	% tikz Semtech2024
...
\section{MAC Komandos \label{lorawan:macCommands}}
	...
	MAC-Befehle, die im \PYTHON{FRMPayload} übertragen werden, müssen hingegen \textbf{immer mit dem \PYTHON{\ac{nwkskey}} verschlüsselt} werden und dürfen die maximale Länge des \PYTHON{FRMPayload} nicht überschreiten (siehe~\autoref{lorawan:frmpayload}) \cite[26]{LoRaAlliance:2020b}.
