...
\subsubsection{MIC-Prüfung und Sicherheitsvalidierung}
	Um ein eingehendes Paket authentifizieren zu können, berechnet der \ac{lns} den \PYTHON{\ac{mic}} anhand des \PYTHON{\ac{nwkskey}} über alle Felder des empfangenen \textbf{MAC Payload} - Frames neu. 
	...
	Anschlie{\ss}end wird dieser berechnete Wert mit dem im Frame enthaltenen \PYTHON{\ac{mic}}-Feld verglichen.
	...
	\end{figure}
	 ...
	 Stimmen beide Werte überein, kann der Server sicherstellen, dass die Nachricht tatsächlich zu diesem Netzwerk gehört und während der Übertragung nicht manipuliert wurde. Nur ein Frame mit gültiger \PYTHON{\ac{mic}}-Prüfung wird weiterverarbeitet.
...
\subsubsection{Beispiel eines \ac{adr}-Algorithmus nach Semtech}
	...
	\begin{figure}
		\centering
		\includegraphics[width=0.9\textwidth]{Flowcharts/ADRFlowchart}
...
\subsection{Entschlüsselung mit dem AppSKey}
	...
	\begin{figure}[H]
		...
		\end{tikzpicture}
		\caption{Schematische Darstellung der \PYTHON{FRMPayload}-Entschlüsselung \cite{Montagny:2022}}
	\end{figure}
...
Alle Applikationsdaten im \PYTHON{FRMPayload} werden Ende-zu-Ende mit dem \PYTHON{\ac{appskey}} verschlüsselt, sodass weder Gateway noch \ac{lns} den Inhalt einsehen oder verändern können. Der \ac{lns} verarbeitet lediglich die für den Netzwerkbetrieb erforderlichen MAC-Felder, führt die \ac{mic}-Prüfung durch und leitet die verschlüsselten Nutzdaten unverändert an den Anwendungserver weiter. Erst dort erfolgt die Entschlüsselung und Interpretation des Payloads \cite{LoRaAlliance:2020b, Montagny:2022}. 

\section{Join Server (JS)}
	Der Join Server ist für die Authentifizierung neuer Endgeräte verantwortlich. Er validiert eingehende \textit{Join Request}-Nachrichten und generiert die Sitzungsschlüssel (\PYTHON{\ac{nwkskey}} und \PYTHON{\ac{appskey}}), die anschlie{\ss}end sicher an \ac{lns} und Anwendungserver übermittelt werden. In privaten oder kleineren Netzwerken kann diese Funktionalität direkt im \ac{lns} integriert sein \cite{Semtech:2024} \cite{LoRaAlliance:2020b}. Es können mehrere \ac{js} an einem \ac{lns} angeschlossen werden und ein \ac{js} kann mehrere \ac{lns} bedienen \cite{LoRaAlliance:2020c}.
\subsection{Validierung von Join-Requests}
	Beim Empfang eines Join-Requests überprüft der Join Server die enthaltenen Geräteidentitäten (\PYTHON{\acs{deveui}}, \PYTHON{\acs{joineui}}). Nur bei erfolgreicher Validierung wird der Join-Accept generiert \cite{Semtech:2024, LoRaAlliance:2020c}.
\subsection{Generierung der Sitzungsschlüssel (NwkSKey, AppSKey)}
	Nach erfolgreicher Prüfung erzeugt der Join Server die beiden Sitzungsschlüssel \PYTHON{\acs{nwkskey}} und \PYTHON{\acs{appskey}}. Diese werden aus dem \PYTHON{\acs{appkey}} des Endgeräts abgeleitet und sind für die Dauer der Aktivierung gültig. Der Join Server ist dabei die einzige Instanz, die den \acs{appkey} kennt und verwalten darf \cite{LoRaAlliance:2020c, Montagny:2022}.
	\begin{figure}[H]
		\centering
		\resizebox{0.8\textwidth}{!}{
			\begin{tikzpicture}
				...
				\LoRaWANBAckendinfrastructure
				\draw[arrowStyle, red, <-] ($(NS.south)+(0,-0.2)$) -- node[midway,left] {\PYTHON{\acs{nwkskey}}} ($(JS.north)+(0,0.2)$);
				\draw[arrowStyle, red, <-] ($(AS.south west)+(0,-0.2)$) -- node[midway,right] {\PYTHON{\acs{appskey}}} ($(JS.north east)+(0,0.2)$);
			\end{tikzpicture}
		}
		\caption{Übermittlung der Sitzungsschlüsseln an \ac{lns} und \ac{as} \cite{Montagny:2022}}
	\end{figure}
\subsection{Sicherer Austausch der Schlüssel mit LNS und AS}
	...
	\begin{itemize}
		\item der \PYTHON{\acs{nwkskey}} an den \ac{lns},
		\item der \PYTHON{\acs{appskey}} an den \ac{as}.
	\end{itemize}
	Da diese Schlüssel unterschiedliche Aufgaben erfüllen, bleiben die Verantwortungsbereiche strikt getrennt. Weder der \ac{lns} noch der \ac{as} erhalten Zugriff auf den \PYTHON{\acs{appkey}} \cite{Montagny:2022, LoRaAlliance:2020c}.
...
\subsection{Ablauf eines Uplinks im Backend}
	\begin{figure}[H]
		...
		\begin{tikzpicture}
			\uplinkProcedure
		\end{tikzpicture}
		\caption{Ablauf eines Uplinks \cite{LoRaAlliance:2020c}}
	\end{figure}
\subsection{Ablauf eines Downlinks}
	\begin{figure}[H]
		...
		\begin{tikzpicture}
			\downlinkProcedure
		\end{tikzpicture}
		\caption{Ablauf eines Downlink \cite{LoRaAlliance:2020c}}
	\end{figure}
