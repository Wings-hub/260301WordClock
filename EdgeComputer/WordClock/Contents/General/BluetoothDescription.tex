...
Bluetooth sensors are devices that utilize Bluetooth technology to wirelessly transmit data over short distances. They are commonly used in various applications including health monitoring, fitness tracking, smart home automation, industrial monitoring, and asset tracking. \cite{PortentaH7BLE:2024}
\newline
\textbf{Key Features:} \newline
\begin{itemize}
...
\item \textbf{Easy Integration:} The Portenta H7 is compatible with the Arduino ecosystem, allowing for easy programming and integration with other Arduino shields and modules, including those utilizing Bluetooth communication. \cite{PortentaH7BLE:2024}
...
\begin{itemize}
	...
	\item The BLE sensor can be programmed using the Arduino IDE with appropriate libraries for BLE communication. \cite{PortentaH7BLE:2024}
...

{
	...
	\ArduinoExternal{}{../../../Code/Arduino/BLE/LedControl/LedControl.ino}
}


This is just a simple example. The necessary libraries such as \textcolor{red}{<ArduinoBLE.h>} should be included, and the BLE device name and services can be defined as per the application requirements. \cite{PortentaH7BLE:2024}
...
In this Example we will enable low energy Bluetooth  on the Portenta H7 to allow an external Bluetooth device to control the built-in LED either by turning it on or off. \cite{PortentaH7BLE:2024}

\subsubsection{Goals}
\begin{itemize}
	\item Enabling Bluetooth Low Energy connectivity on the Portenta H7.
	\item Connecting the Portenta to an external Bluetooth Low Energy Mobile Application (In this case nRF Connect by Nordic Semiconductor). \cite{PortentaH7BLE:2024}
\end{itemize}
...
\begin{itemize}
	...
	These GATT codes are very long, but, in this example, it is always the same code: \cite{PortentaH7BLE:2024}
	...
	\begin{figure}
		\begin{center}
			\includegraphics[width=0.7\linewidth]{../../Images/BLE/NRFConnect2}
			\caption{NRF-Connect}
			\label{NRF-Connect}
		\end{center}
	\end{figure}
	
	Once you have downloaded the nRF application on your mobile device, look for your Portenta in the device list. You may filter the list by "Portenta" to easierly find your board in case you are using nRF Connect. \cite{PortentaH7BLE:2024}
	...
	\begin{enumerate}
		...
		\item Switch to "Bool" type and move the toggle to "True". Confirm the dialog with a tap on "Write" and you should see the built-in LED turned on. If you do the same procedure again but setting the toggle switch to "False", it will turn off the LED.  \cite{PortentaH7BLE:2024}
	\end{enumerate}
	
	\begin{figure}
		\begin{center}
			\includegraphics[width=0.7\linewidth]{../../Images/BLE/BluetoothScan}
			\caption{Bluetooth-scan}
			\label{Bluetooth-scan} \cite{PortentaH7BLE:2024}
		\end{center}
	\end{figure}
	\item \textbf{6. Conclusion:} This example shows how to connect and control the built-in LED using a Bluetooth Low Energy connection. You have learned how a simple Bluetooth Low Energy connection between your Portenta and your cell phone, which has basic communication abilities between the two devices, works. ~\ref{NRF-Connect}	 \cite{PortentaH7BLE:2024}
