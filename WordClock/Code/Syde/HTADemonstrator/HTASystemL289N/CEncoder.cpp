...
CEncoder::CEncoder() {
  ...
  pinEncoderALast = LOW;               // Store previous state for edge detection
...
void CEncoder::setup(uint8_t pinEncoderA, uint8_t pinEncoderB) {
  pinMode(pinEncoderA, INPUT);
  pinMode(pinEncoderB, INPUT);

  pinEncoderALast = digitalRead(pinEncoderA); // Initialize last state
}

// Reads encoder position using rising edge detection on Channel A
int CEncoder::getPosition() {
  valueNow = digitalRead(pinEncoderA);

  // Detect rising edge on channel A
  if ((pinEncoderALast == LOW) && (valueNow == HIGH)) {
  ...
  pinEncoderALast = valueNow; // Update last state
...
int CEncoder::getTestValue() {
  testValue = digitalRead(pinEncoderA);

  // Detect rising edge on channel A
  if ((pinEncoderALast == LOW) && (testValue == HIGH)) {
    // Determine direction using channel B
    if (digitalRead(pinEncoderB) == LOW) {
      encoder0Pos--; // Counter-clockwise
    } else {
  ...
  pinEncoderALast = testValue; // Update last state
...
float CEncoder::getLinearVelocity() {
  ...
  const float ticksPerRevolution = 1600.0f;          // 400 PPR * X4
